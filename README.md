...
| Charles (прокси) | Всё, **кроме** российских IP. Включён в группу, т.к. при проксировании именно процесс Charles открывает соединение к сайту — иначе РФ-трафик браузеров утекает мимо deny. ⚠️ Это блокирует РФ для **любого** трафика через Charles (в т.ч. Firefox/Диск-О, если гнать их через него). |
...
| [generate-ls-ru-rules.sh](generate-ls-ru-rules.sh) | Bash-скрипт: из `merged-ru-cidrs.txt` генерирует `ru-only.lsrules` + `no-ru.lsrules`. Запускается приложением Steward. |
| [merged-ru-cidrs.txt](merged-ru-cidrs.txt) | Стабильный список российских CIDR — вход для генератора. Обновляется автоматически приложением Steward. |
...
| [no-ru.lsrules](no-ru.lsrules) | Little Snitch Rule Group: Safari + Chrome + **Charles** (прокси) ⇒ заблокированы российские IP, **всё остальное разрешено** (группа самодостаточна: deny РФ + allow остального — отдельное ручное allow-правило не нужно). |
...
Стабильный `merged-ru-cidrs.txt` коммитится (его пишет Steward). Приватная копия `ru-whitelist.pac` (с персональными доменами) и промежуточные датированные `merged-ru-cidrs-*.txt` — нет, см. `.gitignore`.
...
**Важно:** снять галочку **Disable new allow rules**, иначе свежеприлетевшие allow-правила окажутся выключенными. Для `no-ru` это критично: группа теперь содержит **allow-правило «всё остальное»** для Safari/Chrome, и если оно прилетит выключенным — браузеры останутся только с `deny` на РФ и снова будут зависеть от внешнего allow-правила (та самая утечка). Если ты раньше держал отдельное ручное «allow all для Chrome/Safari» — **удали его**: группа теперь определяет браузеры полностью.
...
Это нормально и запланировано: в `no-ru.lsrules` для них прописан deny на RU-CIDR (+ allow на всё остальное — группа самодостаточна). Российские сайты открывайте в Firefox (у которого правила наоборот: только РФ).

**Обратная ситуация: Chrome спокойно открывает российский сайт, хотя deny стоит.**
Значит правила не доехали до процесса, который реально открывает соединение. Chrome весь сетевой I/O делает не в главном бинаре, а в отдельном процессе **Google Chrome Helper** (`--utility-sub-type=network.mojom.NetworkService`, Code ID `com.google.Chrome.helper`) — для Little Snitch это другой процесс, и правила на `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome` его не ловят вообще, ни deny, ни allow. Тот же класс проблемы, что и с Charles.

С 2026-08-08 генератор добавляет хелпер в группу автоматически, причём **не по пути, а по Code ID**: путь хелпера содержит номер версии Chrome (`Versions/151.0.7922.72/…`) и меняется при каждом обновлении, так что path-based правило после апдейта молча перестало бы срабатывать. Формат `.lsrules` принимает в ключе `process` значение вида `identifier.DEV_TEAM_ID/IDENTIFIER` вместо абсолютного пути ([документация](https://help.obdev.at/littlesnitch6/adv-lsrules-file-format)) — для хелпера это `identifier.EQHXZ8M8AV/com.google.Chrome.helper`. Такое правило привязано к подписи и обновления Chrome переживает. Генератор читает Code ID прямо из подписи установленного хелпера (`codesign -dv`) и предупреждает, если он разошёлся с константой в скрипте.

Тот же выбор есть в GUI при создании правила вручную — поле **Identification**: `Code ID` (привязка к подписи, правильный вариант), `Path` (ломается при обновлении), `Path (No Identity Check)` (подпись не проверяется — брать не стоит).

Что проверить, если симптом остался:

1. **Ручное `allow any outgoing connection` на Google Chrome Helper** — удалить. Обычно оно появляется, если когда-то ответить на алерт «Any connection / Forever». Оно и пропускает РФ мимо deny.
2. **Проверить, какой именно процесс ходит:** `lsof -nP -i -a -p "$(pgrep -f 'Google Chrome' | tr '\n' ',' | sed 's/,$//')"` — сокеты должны быть у одного PID, и это хелпер. Полный путь и Code ID видны в алерте LS в полях **Established by** и **Code ID**.

У Safari этой проблемы нет: он тоже ходит в сеть через отдельный процесс (`com.apple.WebKit.Networking`), но это системный XPC-сервис, и Little Snitch атрибутирует его приложению-клиенту. Хелпер Chrome форкается самим Chrome, не через XPC, — поэтому учитывается отдельно.
