...
### 6.2. Подключить правила (подписка)

Little Snitch не имеет встроенного фильтра по стране — российские IP-диапазоны раздаются как набор правил из репозитория [app-ru-split](https://github.com/Dabrogelatel/app-ru-split). **Файлы генерируются и обновляются автоматически приложением Steward** (оно ежедневно скачивает свежий CIDR-список из RIPE-снимка, пересобирает `.lsrules` и пушит в репозиторий). Тебе достаточно один раз подписаться — дальше Little Snitch подтягивает обновления сам.
...
- `ru-only.lsrules` — Firefox + Диск-О: allow только российские CIDR + инфраструктура Mozilla/uBlock, deny остальное.
- `no-ru.lsrules` — Safari + Chrome: блокировка российских IP.

**Важно:** снять галочку **Disable new allow rules**, иначе свежеприлетевшие allow-правила окажутся выключенными.

Локальную сеть (loopback / 10.x / 192.168.x) правила специально НЕ открывают — при необходимости добавь private-rule вручную.
...
- **GeoIP-база обновляется автоматически** — приложение Steward ежедневно пересобирает `.lsrules` и пушит в репозиторий; Little Snitch подтягивает по подписке.
